...
Console.WriteLine($"BasePrice: {pizza.BasePrice}");
Console.WriteLine($"ImageName: {pizza.ImageName}"); Console.WriteLine(pizza);
           ...
           Console.WriteLine("THREE GOING ON!!");
            // Handle image upload
            if (ImageFile != null && ImageFile.Length > 0)
            {
                ...
                Console.WriteLine("4 IMAGE SAVED !!");
            }
            Console.WriteLine("5 IMAGE SAVED!!");
            pizza.CreatedAt = DateTime.Now;

            // CrustCategory is already string, no conversion needed
            pizza.Id = GeneratePizzaId();
            Console.WriteLine("6 PIZZA ID!! --> "+pizza.Id);
            _context.Pizzas.Add(pizza);    
            await _context.SaveChangesAsync();
Console.WriteLine("7 IN HERE!!");
               // 🔑 Assign existing crusts to this pizza
            var crusts = await _context.PizzaCrustCategory.ToListAsync();
            foreach (var crust in crusts)
            {
                Console.WriteLine("8 IM HERE !!");
...
Console.WriteLine("9 ALMOST DONE!!");
            await _context.SaveChangesAsync();
Console.WriteLine("10 DONE!!");
        ...
        public async Task<IActionResult> Edit(string? id)
        {
         ...
         //   var crusts = await _context.PizzaCrustCategory.ToListAsync();
          //  ViewBag.CrustCategories = new SelectList(crusts, "Id", "CategoryName", pizza.CrustCategory);
            ...
            [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(string id, Pizza pizza, IFormFile? ImageFile)
        {
        ...
        private string GeneratePizzaId()
        {
            // Get the last pizza ID from the database
            var lastPizza = _context.Pizzas
            ...
            var lastNumber = int.Parse(lastPizza.Id.Split('-')[1]);
            var newNumber = lastNumber + 1;

            return $"PZ-{newNumber:D4}"; // Format as PZ-0001, PZ-0002, etc.
        }
